...
Imports
    ccsd-f12b.h
        Local header file containing the class definition for the CCSDF12B class
    einsums.hpp
...
Functions
    convert_C (MO Coeffs, MO Basis Set, size of basis * 2, bool(with or without frozen core))
        Makes a copy of the Molecular Orbital Coefficient matrix from the OrbitalSpace object to the einsums::Tensor object
        Uses the full space if use_frzn is true, otherwise uses the active space
        Active space includes the occupied orbitals and excludes frozen orbitals
    Set_ERI (Tensor, Desired Slice Indices)
        Copies the ERI (electron repulsion integral) slice tensor from the einsums::Tensor object to the einsums::TensorView object
        For 4 or 3 dimensions with the first dimension of the 3 dimension tensor being the complimentary auxiliary basis set
    two_body_ao_computer (Integral Type, Gaussian Atomic Orbital Tensor, Basis Set Dimensions * 4)
        Computes the two-body integrals using the given integral type (F, FG, F2, Uf, or ERI)
            F: is F^hat_12 #N.B. variable is cgtg# = <pq|f(r_12)|rs> where f(r_12) = e^(-beta r_12) and beta is the F12 exponent
    ...
    three_index_ao_computer (Integral Type, Bpq Tensor, Basis Set Dimensions * 3)
        Similar to the two_body_ao_computer, but computes the three-index integrals in the case of the Density Fitting (DF) method
        Density fitting bases are used for the auxiliary and occupied orbitals i.e. <xi| becomes (A|
    form_oeints (Tensor/Matrix of two Atomic Orbitals)
        Forms the one-electron molecular integrals from atomic orbitals by combining kinetic and potential energy atomic integrals
        for either complimentary auxilliary or occupied orbitals
        Consider a basis change using the coefficients of the basis set bs_ 
    form_teints (Integral Type, Empty electron repulsion integrals Tensor, Complimentary or Occupied for each dimension)
        Initially using two_body_ao_computer to compute the two-electron integrals by type (F, FG, F2, Uf, or ERI) then converts AO to MO
        Calling convert_C (to get MO Coeffs) and einsums::einsum (to matrix multiply into a new basis) to form the two-electron integrals.
        Works in four stages (for each dimension) to produce the ERI tensor, then takes a slice of the tensor
        Slice is correct MO size per dimension (either active/full occupied or complimentary auxilliary)
        Then calls set_ERI to copy the slice to the ERI_Slice tensor sorting the indices
            F: Order goes to (pq|rs) -> (rp|sq)
            K: Order goes to (pq|rs) -> (qs|pr)
            J: Order goes to (pq|rs) -> (sq|rp)
    form_metric_ints (DF ERI Tensor, bool for Fock)
        Computes the metric integrals for the DF (Density Fitting) method
        Frozen core is used unless is_fock is true
        First calculates "G" # i.e. (A|1/r_12|pq) # for DF basis with atomic basis which is converted with convert_C into the MO basis
        Then calculates J_AB = (A|1/r_12|B) and generate J^-1_AB to generate D^A_pq = J^-1_AB G^A_pq and extracts the MO slice
    form_oper_ints (Integral Type, DF ERI Tensor)
        As above to generate BPQ in the MO basis from the DF and atomic basis without the J handling
        The two dimensional "tensor" object version does this explicitly without a call to three_index_ao_computer ie. (A|?|B)
    form_df_teints (Integral Type, ERI Tensor, J^-1_AB Tensor, Complimentary/Orbital/occupied for each dimension)
        Aim is to form the DF two-electron integrals (flexible to occupied/auxilliary bases) the input J^-1_AB is actually J^-1_AB G^A_pq
        First calls form_oper_ints to compute three electron DF integrals for (A|R|PQ) which is einsummed with J^-1_AB
        to produce (PQ|F12|RS) in the MO basis (removing the DF basis)
        If not "G" (1/r_12) then the inverse is added to chem_robust
        (A|R|B) is formed from the form_oper_ints (2 dimensional tensor) and einsummed with J^-1_AB twice and the result take away from chem_robust
        (PQ|F12|RS) = J^-1_AB (A|R|PQ) + (A|R|PQ) J^-1_AB  - J^-1_AB (A|R|B) J^-1_AB 
        Meant to be (84) in Hans-Joachim Werner; Thomas B. Adler; Frederick R. Manby 2007 J. Chem. Phys. 126 164102
        Finally chem_robust is sorted to phys_robust (i.e. (pq|rs) -> (pr|qs))
...
void CCSDF12B::convert_C(einsums::Tensor<double,2> *C, OrbitalSpace bs, const int& dim1, const int& dim2,
                       const bool use_frzn)
{
...
void CCSDF12B::set_ERI(einsums::TensorView<double, 4>& ERI_Slice, einsums::Tensor<double, 4> *Slice)
{
...
void CCSDF12B::set_ERI(einsums::TensorView<double, 3>& ERI_Slice, einsums::Tensor<double, 3> *Slice)
{
...
void CCSDF12B::two_body_ao_computer(const std::string& int_type, einsums::Tensor<double, 4> *GAO,
...
void CCSDF12B::three_index_ao_computer(const std::string& int_type, einsums::Tensor<double, 3> *Bpq,
                                     std::shared_ptr<BasisSet> bs1, std::shared_ptr<BasisSet> bs2)
{
...
void CCSDF12B::form_oeints(einsums::Tensor<double, 2> *h)
{ 
...
void CCSDF12B::form_teints(const std::string& int_type, einsums::Tensor<double, 4> *ERI, std::vector<char> order)
{
    using namespace einsums;
...
void CCSDF12B::form_metric_ints(einsums::Tensor<double, 3> *DF_ERI, bool is_fock)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
...
void CCSDF12B::form_oper_ints(const std::string& int_type, einsums::Tensor<double, 3> *DF_ERI)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
...
void CCSDF12B::form_oper_ints(const std::string& int_type, einsums::Tensor<double, 2> *DF_ERI)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
...
void CCSDF12B::form_df_teints(const std::string& int_type, einsums::Tensor<double, 4> *ERI,
...
void DiskCCSDF12B::set_ERI(einsums::DiskView<double, 2, 4>& ERI_Slice, einsums::TensorView<double, 2>& Slice)
{
    using namespace einsums;
...
void DiskCCSDF12B::form_oeints(einsums::DiskTensor<double, 2> *h)
{
    using namespace einsums;
...
void DiskCCSDF12B::form_teints(const std::string& int_type, einsums::DiskTensor<double, 4> *ERI)
{
    using namespace einsums;
    using namespace tensor_algebra;
    using namespace tensor_algebra::index;
...
void DiskCCSDF12B::form_df_teints(const std::string& int_type, einsums::DiskTensor<double, 4> *ERI,
